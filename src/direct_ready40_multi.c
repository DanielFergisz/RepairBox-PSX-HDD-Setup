...
static int dr40_multi_exact_system_layout(
    const storage_device_result_t *device)
{
                                ...
                                u32 native_max, u32 *end, u32 *tail)
{
    ...
    if (native_max <= 0x04CC17C8u + 0x800u)
        return 0;
    expected_end = (u64)native_max - 0x800u;
    expected_size = expected_end - 0x04CC17C8u;
    ...
    if (tail != NULL)
        *tail = calculated_end <= native_max
                    ? native_max - (u32)calculated_end : 0;
    return device->entry_count == DR40_DVR_COUNT &&
        ...
        entry_matches(device, "__xcontents", 0x04CC17C8u,
                      (u32)expected_size) &&
        calculated_end == expected_end &&
        native_max - (u32)calculated_end == 0x800u;
}
...
static int hardware_profile(const dr40_snapshot_t *snapshot)
{
    capacity_profile_t profile = capacity_profile_detect(
        (u32)snapshot->getmaxlba48, snapshot->islba48);

    return capacity_profile_is_supported(profile) &&
...
static void __attribute__((unused)) dr40_multi_initialize_unused(
    dr40_result_t *result, const storage_diagnostics_t *diagnostics)
{
    unsigned int index;
                        ...
                        dr40_progress_callback_t progress, void *context)
{
    ...
    result->system_layout_valid =
        ...
        dr40_multi_exact_system_layout(&diagnostics->hdd);
    ...
    refresh_after(diagnostics, result);
    result->dvr_layout_valid = dr40_multi_exact_dvr_layout(
        &diagnostics->dvr_hdd, (u32)result->before.getmaxlba48,
        &result->xcontents_end,
    ...
    result->system_layout_valid =
        dr40_multi_exact_system_layout(&diagnostics->hdd);
    result->dvr_layout_valid = dr40_multi_exact_dvr_layout(
        &diagnostics->dvr_hdd, (u32)result->after.getmaxlba48,
        &result->xcontents_end,
        &result->tail_reserved_sectors);
    ...
    result->direct_ready40_storage_valid =
        ...
        capacity_profile_is_supported(capacity_profile_detect(
            (u32)result->after.getmaxlba48,
            result->after.islba48)) &&
...
static void __attribute__((unused)) dr40_multi_release_unused(
    dr40_result_t *result)
{
    installer_release(&result->installer);
}
